...
这一页专门说明首页里用到的界面图分别在展示什么，避免截图只是“摆图”。

## 1. System Flow

![System flow](../images/system-flowchart-original.png)

这张图来自我原始课程说明书中的系统流程图，覆盖了：
...
它不是后来为了仓库重新补画的图，而是我当时用于解释系统整体结构的原始图。
...
这个版本里，传感器数据仍然以模拟为主，但这正是我当时为了优先验证通信链路而做的取舍。
...
- 按用户、传感器类型、数值范围、时间范围进行筛选
- 回看传感器数据记录
- 证明系统不只“能发数据”，还能做基础留存和查询
