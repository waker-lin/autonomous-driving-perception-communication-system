...
void DataSearch::OnBnClickedButton1()
{
	...
	else if (user.empty() && sensor.empty() && time.empty() && maxData.IsEmpty() && minData.IsEmpty())
	{
		AfxMessageBox(_T("请输入查找条件"));
	}
	else
		performSearch(user, sensor, time, _tstof(minData), _tstof(maxData));
...
void DataSearch::performSearch(const std::string& userQuery, const std::string& sensorTypeQuery,
	const std::string& timeQuery, double minData, double maxData) {
	// 清空上次查询结果
	// 清空 listcontrol 中的内容
	m_listCtrl.DeleteAllItems();
	bool match = true, found = false;
	CString Data,Num;
	/*Data.Format(_T("nihao"));
	m_listCtrl.SetItemText(1, 1, Data);
	int rowIndex = m_listCtrl.InsertItem(0, _T("")); // Insert at the first row (index 0)
	CString Data;
	Data.Format(_T("nihao"));
	m_listCtrl.SetItemText(rowIndex, 0, Data);*/
	...
	for (const auto& data : searchDataList) {
		...
		/*if (data.user == "Car1")
		{
			//AfxMessageBox(_T("user"));
		}
		if (userQuery == "Car1")
		{
			//AfxMessageBox(_T("Car2"));
		}
		if (data.sensorType == "速度传感器")
		{
			AfxMessageBox(_T("user"));
		}
		if (sensorTypeQuery == "速度传感器")
		{
			AfxMessageBox(_T("Car2"));
		}
		if (data.sensorType == sensorTypeQuery)
		{
			AfxMessageBox(_T("Car3"));
		}*/
