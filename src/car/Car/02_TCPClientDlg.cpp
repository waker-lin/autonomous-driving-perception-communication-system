...
BOOL CMy02_TCPClientDlg::OnInitDialog()
{
	...
	m_hIconRed = (HICON)LoadImage(AfxGetInstanceHandle(), MAKEINTRESOURCE(AFX_ID_PREVIEW_PREV), IMAGE_ICON, 40, 40, LR_DEFAULTCOLOR);
	m_hIconGreen = (HICON)LoadImage(AfxGetInstanceHandle(), MAKEINTRESOURCE(IDB_PNG1), IMAGE_ICON, 40, 40, LR_DEFAULTCOLOR);
	...
	CImage image;
	image.Load(_T("D:\\HuaweiMoveData\\Users\\HUAWEI\\Desktop\\excel\\Zn7JFMZCcO.png"));
...
{
	// TODO: 在此处添加实现代码.
	handle_Thread = CreateThread(NULL, 0, process1, this, 0, 0);
...
void CMy02_TCPClientDlg::OnBnClickedButton11()
{
	...
	for (auto it = m_Data.begin(); it != m_Data.end(); it++) {
		char szBuf[256] = { 0 };
		//msg.Format(_T("%s"), it->Sensor);
		//AfxMessageBox(msg);//在string转cstring出现乱码

		// 获取消息的字符数组指针，并复制消息内容到缓冲区
		//strncpy_s(szBuf, CT2A(msg.GetBuffer()), sizeof(szBuf) - 1);//这个是真不容易，尝试了好久啊
		// strncpy_s(szBuf, CT2AEX< CP_UTF8 >(msg.GetBuffer()), sizeof(szBuf) - 1);
		if (it->Sensor == "毫米波雷达")
		{
			iWrite = m_connSocket.Send("速度:", 5);
		}
		else if (it->Sensor == "激光雷达")
		{
			iWrite = m_connSocket.Send("车距:", 5);
		}
		else
		{
			iWrite = m_connSocket.Send("行人:", 5);
		}
		strncpy_s(szBuf, sizeof(szBuf), it->dData.c_str(), sizeof(it->dData) - 1);
		iWrite = m_connSocket.Send(szBuf, strlen(szBuf));

		strncpy_s(szBuf, sizeof(szBuf), it->unit.c_str(), sizeof(it->unit) - 1);
		iWrite = m_connSocket.Send(szBuf, strlen(szBuf));
		//iWrite = m_connSocket.Send("\r\n", 6);
		...
		// 发送完整消息给客户端
		//int iWrite = m_connSocket.Send(szBuf, strlen(szBuf));

		//AfxMessageBox((LPCTSTR)szBuf);
		//AfxMessageBox();
		//int iWrite = m_connSocket.Send("nihao", 6);
